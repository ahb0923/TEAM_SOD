...
public class StatController : MonoBehaviour
{
    ...
    //『효빈』ScriptableObject를 활용한 초기화는 json data가 없을 경우에 실행
    ...
    public void InitStat()
    {
        ...
        Crit_Chance = playerData.CritChance;
        Crit_Multiply = playerData.CritMultiply;
        Invinsible_Duration = playerData.InvinsibleDuration;
        Is_Invinsible = playerData.Is_Invinsible;
    }
    ...
    /// 『효빈』굳이 구조체로 만들 필요 없을 것 같습니다, 클래스나 메소드로 처리하는 편이 더 좋을 것 같아요.
    /// </summary>
    ...
    public DamageResult FinalDamageCalculator(IDamageInfo damageinfo)
    {
        ...
        bool _is_Crit = (random <= damageinfo.Critical_Chance) ? true : false; // 크리 여부 판정
